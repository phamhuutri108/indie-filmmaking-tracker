...
// .wrangler/tmp/bundle-4onEwQ/checked-fetch.js
...
var getAssetFromKV = /* @__PURE__ */ __name2(async (event, options) => {
  ...
  const ASSET_MANIFEST2 = parseStringAsObject(
...
async function checkMonitorCommands(env) {
  ...
  for (const cmd of commands.results) {
    ...
    const daysUntil = Math.ceil(
      (deadline.getTime() - today.getTime()) / (1e3 * 60 * 60 * 24)
    );
...
__name(checkMonitorCommands, "checkMonitorCommands");
async function triggerAlert(env, cmd, daysUntil) {
  const subject = daysUntil === 1 ? `[IFT] TOMORROW: ${cmd.ref_name || cmd.target_name}` : `[IFT] ${daysUntil} days left: ${cmd.ref_name || cmd.target_name}`;
  await sendEmail(env, {
    to: env.ALERT_EMAIL,
    subject,
...
__name(triggerAlert, "triggerAlert");
async function sendDailyDigest(env) {
  ...
  await sendEmail(env, {
    ...
    html: buildDigestEmail(upcoming.results)
  });
}
...
function buildAlertEmail(cmd, daysUntil) {
  ...
  return `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <h2 style="color: #e53e3e;">\u26A0\uFE0F Deadline Alert / Nh\u1EAFc nh\u1EDF h\u1EA1n ch\xF3t</h2>
      <p><strong>${name}</strong></p>
      <p>Deadline: <strong>${deadline}</strong> (${daysUntil === 1 ? "TOMORROW / NG\xC0Y MAI" : `${daysUntil} days / ${daysUntil} ng\xE0y`})</p>
...
__name(buildAlertEmail, "buildAlertEmail");
function buildDigestEmail(items) {
  ...
  return `
    <div style="font-family: sans-serif; max-width: 700px; margin: 0 auto; padding: 20px;">
      <h2>\u{1F3AC} IFT Weekly Digest</h2>
      <p>Upcoming deadlines in the next 30 days / H\u1EA1n ch\xF3t trong 30 ng\xE0y t\u1EDBi:</p>
...
__name(buildDigestEmail, "buildDigestEmail");
...
app.get("/api/festivals/:id", async (c) => {
  const row = await c.env.DB.prepare(`SELECT * FROM festivals WHERE id = ?`).bind(c.req.param("id")).first();
...
app.get("/api/funds", async (c) => {
  ...
  if (region_focus) {
    query += ` AND region_focus = ?`;
    params.push(region_focus);
  }
...
app.post("/api/monitors", async (c) => {
  const body = await c.req.json();
  const { target_url, target_name, monitor_type, ref_id, ref_table, watch_for, alert_days_before } = body;
  const result = await c.env.DB.prepare(
    `INSERT INTO monitor_commands (target_url, target_name, monitor_type, ref_id, ref_table, watch_for, alert_days_before)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).bind(target_url, target_name, monitor_type, ref_id, ref_table, watch_for, alert_days_before ?? 7).run();
  return c.json({ id: result.meta.last_row_id }, 201);
});
...
// .wrangler/tmp/bundle-4onEwQ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-4onEwQ/middleware-loader.entry.ts
