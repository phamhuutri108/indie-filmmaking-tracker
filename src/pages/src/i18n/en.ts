export const en = {
  nav: {
    dashboard: 'Dashboard',
    festivals: 'Festivals',
    funds: 'Funds & Grants',
    education: 'Education & Residency Hub',
    festivalInformation: 'Festival Information',
    user: 'User',
    monitors: 'Monitors',
    yourFilms: 'Your Films',
    submissions: 'Submissions',
    watchlist: 'Watchlist',
  },
  dashboard: {
    title: 'Film Intelligence Dashboard',
    subtitle: 'Upcoming deadlines in the next 30 days',
    noItems: 'No upcoming deadlines.',
    type: {
      festival: 'Festival',
      fund: 'Fund',
      education: 'Education',
    },
    stats: {
      festivals: 'Festivals',
      funds: 'Funds & Grants',
      education: 'Education',
      upcoming7: 'Due in 7 days',
      upcoming30: 'Due in 30 days',
      films: 'My Films',
      submissions: 'Submissions',
    },
  },
  festivals: {
    title: 'Festival Tracker',
    subtitle: 'International film festivals & deadlines',
    earlyDeadline: 'Early Deadline',
    regularDeadline: 'Deadline',
    lateDeadline: 'Late Deadline',
    category: 'Category',
    tier: 'Tier',
    country: 'Country',
    city: 'City',
    entryFee: 'Entry Fee',
    festivalDates: 'Festival Dates',
    notification: 'Notification Date',
    source: 'Source',
    filmfreeway: 'FilmFreeway',
    search: 'Search festivals...',
    addFestival: 'Add Festival',
    noResults: 'No festivals found.',
    form: {
      title: 'Add Festival',
      name: 'Festival Name *',
      nameVi: 'Name (Vietnamese)',
      country: 'Country',
      city: 'City',
      website: 'Website URL',
      filmfreeway: 'FilmFreeway URL',
      category: 'Category',
      tier: 'Tier',
      earlyDeadline: 'Early Deadline',
      regularDeadline: 'Regular Deadline *',
      lateDeadline: 'Late Deadline',
      entryFeeEarly: 'Entry Fee Early',
      entryFeeRegular: 'Entry Fee Regular',
      description: 'Description',
    },
  },
  funds: {
    title: 'Fund & Grant Radar',
    subtitle: 'International film funds, grants & financing',
    maxAmount: 'Max Amount',
    deadline: 'Deadline',
    type: 'Type',
    focus: 'Focus',
    region: 'Region',
    organization: 'Organization',
    eligibility: 'Eligibility',
    search: 'Search funds...',
    addFund: 'Add Fund',
    noResults: 'No funds found.',
    lastChecked: 'Last checked',
    refresh: 'Refresh from web',
    refreshing: 'Checking...',
    form: {
      title: 'Add Fund / Grant',
      name: 'Fund Name *',
      nameVi: 'Name (Vietnamese)',
      organization: 'Organization',
      country: 'Country',
      website: 'Website URL',
      type: 'Type',
      focus: 'Focus',
      region: 'Region Focus',
      maxAmount: 'Max Amount (USD)',
      deadline: 'Deadline',
      eligibility: 'Eligibility',
      description: 'Description',
    },
  },
  education: {
    title: 'Education & Residency Hub',
    subtitle: 'Labs, residencies, workshops & scholarships',
    deadline: 'Deadline',
    duration: 'Duration',
    type: 'Type',
    organization: 'Organization',
    country: 'Country',
    stipend: 'Stipend',
    coversTravel: 'Covers Travel',
    coversAccommodation: 'Covers Accommodation',
    programDates: 'Program Dates',
    eligibility: 'Eligibility',
    search: 'Search programs...',
    addEducation: 'Add Program',
    noResults: 'No programs found.',
    form: {
      title: 'Add Education / Residency',
      name: 'Program Name *',
      nameVi: 'Name (Vietnamese)',
      organization: 'Organization',
      country: 'Country',
      city: 'City',
      website: 'Website URL',
      type: 'Type',
      duration: 'Duration',
      deadline: 'Deadline',
      stipend: 'Stipend (USD)',
      coversTravel: 'Covers Travel',
      coversAccommodation: 'Covers Accommodation',
      eligibility: 'Eligibility',
      description: 'Description',
    },
  },
  monitors: {
    title: 'Monitor Commands',
    subtitle: 'Set alerts for deadlines you want to track',
    addMonitor: 'Add Monitor',
    alertDays: 'days before',
    active: 'Active',
    deactivate: 'Deactivate',
    url: 'URL to monitor',
    name: 'Name (optional)',
    watchFor: 'Watch for',
    noMonitors: 'No monitors set up yet.',
    linkRecord: 'Link to record (optional)',
    linkedTo: 'Linked to',
    selectCategory: 'Select category',
    selectRecord: 'Select record',
    deadline: 'Deadline',
  },
  films: {
    title: 'My Films',
    subtitle: "Tri Pham's film projects",
    addFilm: 'Add Film',
    noFilms: 'No films yet. Add your first film!',
    search: 'Search films...',
    duration: 'Duration',
    minutes: 'min',
    logline: 'Logline',
    director: 'Director',
    producer: 'Producer',
    status: {
      'in-production': 'In Production',
      completed: 'Completed',
      released: 'Released',
    },
    genre: 'Genre',
    year: 'Year',
    form: {
      title: 'Add Film',
      editTitle: 'Edit Film',
      name: 'Title *',
      nameVi: 'Title (Vietnamese)',
      year: 'Year',
      genre: 'Genre',
      duration: 'Duration (min)',
      logline: 'Logline',
      director: 'Director',
      producer: 'Producer',
      status: 'Status',
      posterUrl: 'Poster URL',
      trailerUrl: 'Trailer URL',
      notes: 'Notes',
    },
  },
  submissions: {
    title: 'Submission Tracker',
    subtitle: 'Track film submissions to festivals, funds & education',
    addSubmission: 'Log Submission',
    noSubmissions: 'No submissions yet.',
    search: 'Search submissions...',
    film: 'Film',
    target: 'Festival / Fund / Program',
    platform: 'Platform',
    submittedAt: 'Submitted',
    deadline: 'Deadline',
    entryFee: 'Entry Fee Paid',
    resultDate: 'Result Date',
    status: {
      draft: 'Draft',
      submitted: 'Submitted',
      accepted: 'Accepted',
      rejected: 'Rejected',
      waitlisted: 'Waitlisted',
      withdrawn: 'Withdrawn',
    },
    statusColor: {
      draft: '#718096',
      submitted: '#004aad',
      accepted: '#38A169',
      rejected: '#E53E3E',
      waitlisted: '#D69E2E',
      withdrawn: '#A0AEC0',
    },
    form: {
      title: 'Log Submission',
      film: 'Film *',
      refTable: 'Category *',
      refId: 'Festival / Fund / Program *',
      platform: 'Submission Platform',
      submittedAt: 'Submitted At',
      entryFee: 'Entry Fee Paid (USD)',
      status: 'Status',
      resultDate: 'Result Date',
      notes: 'Notes',
      url: 'Submission URL',
    },
    refTable: {
      festivals: 'Festival',
      funds_grants: 'Fund / Grant',
      education_residency: 'Education / Residency',
    },
  },
  calendar: {
    export: 'Export to Calendar',
    exportIcs: 'Download ICS',
    exportHint: 'Import into Google Calendar, Apple Calendar, or any ICS-compatible app.',
  },
  watchlist: {
    title: 'Watchlist',
    subtitle: 'Starred festivals, funds & programs',
    noItems: 'Your watchlist is empty. Star items from Festivals, Funds, or Education tabs.',
    remove: 'Remove',
    addedOn: 'Starred on',
    notes: 'Notes',
  },
  sections: {
    title: 'Sections',
    addSection: 'Add Section',
    noSections: 'No sections yet.',
    sectionName: 'Section Name *',
    sectionNameVi: 'Section Name (Vietnamese)',
    general: 'General Submissions',
    deleteSection: 'Delete Section',
    category: 'Category',
    earlyDeadline: 'Early Deadline',
    regularDeadline: 'Regular Deadline',
    lateDeadline: 'Late Deadline',
    currency: 'Currency',
    entryFeeEarly: 'Entry Fee Early',
    entryFeeRegular: 'Entry Fee Regular',
    entryFeeLate: 'Entry Fee Late',
    filmfreeway: 'FilmFreeway URL',
    shortFilmDuration: 'Short film duration',
    shortFilmMinMin: 'Short Film Min Duration (min)',
    shortFilmMaxMin: 'Short Film Max Duration (min)',
  },
  prestige: {
    label: 'Credibility',
    filterLabel: 'All credibility',
    'a-list': 'A-List International',
    recognized: 'Recognized',
    credible: 'Credible',
    unverified: 'Unverified',
    'not-recommended': 'Not Recommended',
    signals: {
      'fiapf-accredited': 'FIAPF Accredited',
      'oscar-qualifying': 'Oscar Qualifying',
      'bafta-qualifying': 'BAFTA Qualifying',
      'physical-venue': 'Physical Venue',
      'long-history': 'Long History',
      'industry-press': 'Industry Press',
      'cash-prize': 'Cash Prize',
      'distribution-deal': 'Distribution Deal',
      'prestigious-jury': 'Prestigious Jury',
      'government-backed': 'Government Backed',
      'university-affiliated': 'University Affiliated',
      'major-fund': 'Major Fund',
      'international-scope': 'International Scope',
      'major-festival': 'Major Festival',
      'prestigious-fund': 'Prestigious Fund',
      'prestigious-lab': 'Prestigious Lab',
    },
  },
  categories: {
    filterLabel: 'All genres',
    documentary: 'Documentary',
    narrative: 'Narrative',
    short: 'Short Film',
    feature: 'Feature Film',
    animation: 'Animation',
    experimental: 'Experimental',
    student: 'Student Film',
  },
  tiers: {
    'a-list': 'A-List',
    regional: 'Regional',
    emerging: 'Emerging',
  },
  common: {
    viewDetails: 'View Details',
    website: 'Website',
    deadline: 'Deadline',
    daysLeft: 'days left',
    expired: 'EXPIRED',
    tomorrow: 'TOMORROW',
    today: 'TODAY',
    loading: 'Loading...',
    error: 'Error loading data',
    noData: 'No data available',
    add: 'Add',
    save: 'Save',
    cancel: 'Cancel',
    close: 'Close',
    search: 'Search',
    all: 'All',
    details: 'Details',
    edit: 'Edit',
    yes: 'Yes',
    no: 'No',
    usd: 'USD',
    status: {
      active: 'Active',
      closed: 'Closed',
      cancelled: 'Cancelled',
    },
  },
  festivalProfile: {
    backToFestivals: '← Back to Festivals',
    tabOverview: 'Overview',
    tabSections: 'Sections & Deadlines',
    tabMyStatus: 'My Status',
    tabHistory: 'History',
    about: 'About',
    whatTheyLookFor: 'What They Look For',
    eligibility: 'Eligibility Requirements',
    prizes: 'Prizes & Benefits',
    industryPresence: 'Industry Presence',
    tips: 'Tips for Applying',
    pastSelections: 'Past Notable Selections',
    noPastFilms: 'No notable past selections available for this festival.',
    pastFilmsNotice: 'AI-generated from public data. May be incomplete for smaller festivals.',
    acceptanceRate: 'Acceptance Rate',
    usefulLinks: 'Useful Links',
    regenerate: 'Regenerate Profile',
    regenerating: 'Generating...',
    generatedBy: 'Profile generated by AI',
    cached: 'Cached',
    confidenceHigh: 'High confidence — well-documented festival',
    confidenceMedium: 'Medium confidence — some information may be incomplete',
    confidenceLow: 'Low confidence — limited public data available for this festival',
    watchlistStatus: 'Watchlist',
    inWatchlist: 'In your watchlist',
    notInWatchlist: 'Not in watchlist',
    addToWatchlist: '+ Add to Watchlist',
    removeFromWatchlist: 'Remove from Watchlist',
    myMonitors: 'Active Monitors',
    noMonitors: 'No monitors set up for this festival.',
    addMonitor: '+ Set up a monitor',
    mySubmissions: 'My Submissions',
    noSubmissions: 'No submissions logged for this festival.',
    logSubmission: '+ Log a submission',
    viewFullProfile: 'View Full Profile →',
    year: 'Year',
    filmTitle: 'Title',
    director: 'Director',
    country: 'Country',
    award: 'Award',
    links: 'Links',
    approxRate: 'approx.',
    noInsights: 'No profile data yet.',
    loadingInsights: 'Generating profile... (about 15–30 seconds)',
  },
} as const;

export type I18nEn = typeof en;
